...
func TestAppearanceContractsIgnoreLegacySkinFields(t *testing.T) {
	value := defaultAppearanceSetting()
	legacy := []byte(`{"schemaVersion":7,"brandName":"旧品牌","skinId":"custom-old","skinThemes":[{"id":"custom-old"}],"activeSkin":{"id":"custom-old"}}`)
	if err := json.Unmarshal(legacy, &value); err != nil {
		t.Fatal(err)
	}
	value.SchemaVersion = appearanceSchemaVersion

	adminJSON, err := json.Marshal(AdminAppearanceSetting{AppearanceSetting: value})
	if err != nil {
		t.Fatal(err)
	}
	publicJSON, err := json.Marshal(publicAppearanceSetting(nil, value))
	if err != nil {
		t.Fatal(err)
	}
	for _, encoded := range [][]byte{adminJSON, publicJSON} {
...
func TestAppearanceBackfillsVersionEightFieldsAndIgnoresLegacySkin(t *testing.T) {
	svc, db, _, admin := newAppearanceTestService(t)
	legacy := model.SystemSetting{Key: appearanceSettingKey, ValueJSON: `{"schemaVersion":7,"brandName":"旧品牌","skinId":"custom-old","skinThemes":[{"id":"custom-old"}]}`}
	...
	if _, err := svc.UpdateAppearance(admin, AppearanceSetting{BrandName: "旧品牌", BrandSlug: defaultAppearanceBrandSlug, AuthHeroTitle: defaultAppearanceHeroTitle, LogoFrameEnabled: true, AuthVideoAutoplay: true}); err != nil {
		t.Fatal(err)
	}
...
func TestUpdateAppearancePersistsAuditsAndUsesVersionedPublicAssets(t *testing.T) {
	...
	if !updated.Configured || updated.BrandName != "HIMA Studio" || updated.BrandSlug != "hima-studio" || updated.Public.BrandSlug != "hima-studio" || updated.AuthHeroTitle != "把灵感，\n变成可见的故事。" || updated.Public.AuthHeroDescription != "从画布开始，持续推进你的创作。" || !updated.Public.LogoConfigured || !updated.Public.DarkLogoConfigured || updated.Public.LogoFrameEnabled || !updated.Public.AuthVideoConfigured || !updated.Public.AuthVideoPosterConfigured || updated.Public.AuthVideoAutoplay || updated.Public.SEOTitle != "HIMA Studio - AI 影视工作台" || updated.Public.SEOKeywords != "AI 影视,短剧,画布" || !updated.Public.ICPFilingEnabled || updated.Public.ICPFilingNumber != "蜀ICP备2026000000号-1" {
	...
	if strings.Contains(string(encodedPublic), "skinId") || strings.Contains(string(encodedPublic), "activeSkin") || strings.Contains(string(encodedPublic), "skinThemes") {
		t.Fatalf("public appearance contains removed skin fields: %s", encodedPublic)
	}
...
func TestAppearanceReusesSingleLogoAcrossThemes(t *testing.T) {
	...
	lightOnly, err := svc.UpdateAppearance(admin, AppearanceSetting{BrandName: "HIMA Studio", BrandSlug: "hima-studio", AuthHeroTitle: defaultAppearanceHeroTitle, LogoResourceID: "only-light", LogoFrameEnabled: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	darkOnly, err := svc.UpdateAppearance(admin, AppearanceSetting{BrandName: "HIMA Studio", BrandSlug: "hima-studio", AuthHeroTitle: defaultAppearanceHeroTitle, DarkLogoResourceID: "only-dark", LogoFrameEnabled: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestUpdateAppearanceRejectsAnotherUsersNewResource(t *testing.T) {
	...
	_, err := svc.UpdateAppearance(admin, AppearanceSetting{BrandName: "HIMA Studio", BrandSlug: "hima-studio", AuthHeroTitle: defaultAppearanceHeroTitle, LogoResourceID: resource.ID})
...
func TestOpenAppearanceAssetOnlyServesConfiguredSlot(t *testing.T) {
	...
	if _, err := svc.UpdateAppearance(admin, AppearanceSetting{BrandName: "HIMA Studio", BrandSlug: "hima-studio", AuthHeroTitle: defaultAppearanceHeroTitle, LogoResourceID: resource.ID}); err != nil {
		t.Fatal(err)
	}
...
func TestUpdateAppearanceValidatesLoginCopy(t *testing.T) {
	...
	_, err := svc.UpdateAppearance(admin, AppearanceSetting{BrandName: "HIMA Studio", BrandSlug: "hima-studio", AuthHeroTitle: ""})
	if err == nil || !strings.Contains(err.Error(), "主标题") {
		t.Fatalf("empty title error = %v", err)
	}
	_, err = svc.UpdateAppearance(admin, AppearanceSetting{BrandName: "HIMA Studio", BrandSlug: "hima-studio", AuthHeroTitle: defaultAppearanceHeroTitle, AuthHeroDescription: strings.Repeat("文", 161)})
	if err == nil || !strings.Contains(err.Error(), "160") {
		t.Fatalf("long description error = %v", err)
	}
	_, err = svc.UpdateAppearance(admin, AppearanceSetting{BrandName: "HIMA Studio", BrandSlug: "hima-studio", AuthHeroTitle: "标题\t注入"})
	if err == nil || !strings.Contains(err.Error(), "控制字符") {
		t.Fatalf("control character error = %v", err)
	}
	_, err = svc.UpdateAppearance(admin, AppearanceSetting{BrandName: "HIMA Studio", BrandSlug: "HIMA Studio", AuthHeroTitle: defaultAppearanceHeroTitle})
	if err == nil || !strings.Contains(err.Error(), "英文品牌标识") {
		t.Fatalf("invalid slug error = %v", err)
	}
	_, err = svc.UpdateAppearance(admin, AppearanceSetting{BrandName: "HIMA Studio", BrandSlug: "hima-studio", AuthHeroTitle: defaultAppearanceHeroTitle, ICPFilingEnabled: true})
